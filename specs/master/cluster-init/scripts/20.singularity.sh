...
case ${Singularity_VERSION} in
     ...
     3.5.* )
           if [[ ! -f ${APPSDIR}/singularity-${Singularity_VERSION}-1.el7.x86_64.rpm ]]; then
              jetpack download "singularity-${Singularity_VERSION}-1.1.el7.x86_64.rpm" ${APPSDIR}
              chown ${CUSER}:${CUSER} ${APPSDIR}/singularity-${Singularity_VERSION}-1.1.el7.x86_64.rpm
           fi
           if [[ ! -f /bin/singularity ]]; then
              rpm -ivh ${APPSDIR}/singularity-${Singularity_VERSION}-1.1.el7.x86_64.rpm
           fi
     ;;
esac
