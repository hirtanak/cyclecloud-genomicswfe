...
bash 'Install cyclecloud python api' do
  code <<-EOH
    jetpack download --project genomicswfe cyclecloud-api-7.9.2.tar.gz cyclecloud-api.tar.gz || exit 1;
...
bash 'Install job_submit/cyclecloud' do
  code <<-EOH
    jetpack download --project genomicswfe job_submit_cyclecloud_#{node[:platform]}_#{slurmver}.so  /usr/lib64/slurm/job_submit_cyclecloud.so || exit 1;
